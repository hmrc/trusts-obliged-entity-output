...
class IfsTrustDataConnector @Inject() (http: HttpClientV2, config: AppConfig)(using ec: ExecutionContext)
  ...
  def getTrustJson(identifier: Identifier): Future[TrustDataResponse] = {
    ...
    http
      .get(url"$url")
      .execute[TrustDataResponse](using TrustDataResponse.ifsHttpReads(identifier), ec)
  }
